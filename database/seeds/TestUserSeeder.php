...
class TestUserSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $domains = explode(' ', 'com cn xin xyz club shop top wang win site vip store net bid cc ltd ren lol mom online tech biz red website space link news date loan mobi press video market live studio help info click pics photo trade vc party rocks band gift wiki design software social lawyer engineer org comcn netcn orgcn govcn name tv me asia co work game games pro kim science 中国 公司 网络 pub');
        for ($i = 0; $i < 100; $i++){
            $first_name = $first_names[rand(0, count($first_names)-1)];
            $last_name = $last_names[rand(0, count($last_names)-1)];
            $name = $first_name . $last_name;
            $email = \Illuminate\Support\Str::random(rand(3,10)) . "@" . \Illuminate\Support\Str::random(rand(2,5)) .'.'. $domains[rand(0, count($domains)-1)];
